...
let kClientName = "6AB64hcB"
let kClientKey  = "6gSuV295YD86Mq4d86zEsx8C839uMVVjfXm9N4wr6DRuhTHpDU97NFyKtfZncUq8"
...
class CardDetailVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.headerView.backgroundColor = UIColor.init(red: 198.0/255.0, green: 147.0/255.0, blue: 211.0/255.0, alpha: 1.0)
    ...
    func updateTokenButton(_ isEnable: Bool) {
        ...
        if isEnable {
            self.getTokenButton.backgroundColor = UIColor.init(red: 198.0/255.0, green: 147.0/255.0, blue: 211.0/255.0, alpha: 1.0)
        } else {
            self.getTokenButton.backgroundColor = UIColor.init(red: 198.0/255.0, green: 147.0/255.0, blue: 211.0/255.0, alpha: 0.2)
        }
    }
    ...
    func getToken() {
        ...
        handler!.getTokenWithRequest(request, successHandler: { (inResponse:AcceptSDKTokenResponse) -> () in
            DispatchQueue.main.async(execute: { [self] in
                ...
                //for succesfull token Get and Payment Done
                self.paymentSuccessService()
                //self.updatePurchaseStatusService_Second(transactionId: "\(inResponse.getOpaqueData().getDataValue())", authCode: "", responseCode: "1", amount: "\(self.selectedAmount)", paymentStatus: "Approved", paymentResponse: "This transaction has been approved.")
            ...
            //for failure Payment
            self.view.makeToast("Payment Fail Tyr Again")
...
//MARK: extension for success payment done
extension CardDetailVC{
    func paymentSuccessService() {
        PKHUD.sharedHUD.contentView = PKHUDProgressView()
        PKHUD.sharedHUD.show()
        
        let url = AppUrl.payment_successURL()
        
        let parameters: [String: Any] = ["user_id" : Defaults[PDUserDefaults.UserID],
                                         "cardnumber":"\(self.cardNumber!)",
                                         "month":"\(self.expirationMonthTextField.text!)",
                                         "year":"20\(self.expirationYearTextField.text!)",
                                         "cvv":"\(self.cardVerificationCodeTextField.text!)",
                                         "amount":self.selectedAmount,
                                         "device" : "ios"]
        
        print("Url_paymentSuccessService_is_here:-" , url)
        print("Param_paymentSuccessService_is_here:-" , parameters)
                ...
                if responseJson["code"] == "200" {
                    if let responseData = response.data {
                        do {
                            ...
                            let dicData = try decodeJSON.decode(PaymentSuccessData.self, from: responseData)
                            
                            print("paymentSuccessService_dicData = \(String(describing: dicData.msg?.first))")
                            if (dicData.msg?.count)! > 0 {
                                let objProfileRes = (dicData.msg!.first)!
                                print("paymentSuccessService_objProfileRes_IS_here",objProfileRes)
                                let transactionId = "\(objProfileRes.transactionID!)"
                                let authCode = "\(objProfileRes.authCode!)"
                                let responseCode = "\(objProfileRes.responseCode!)"
                                let amount = "\(objProfileRes.amount!)"
                                let paymentStatus = "\(objProfileRes.paymentStatus!)"
                                let paymentResponse = "\(objProfileRes.paymentResponse!)"
                                
                                if responseCode == "1"{
                                    //Make Toast
                                    self.view.makeToast("\(objProfileRes.paymentResponse!)")
                                    
                                    //call API update_Purchase_Status
                                    self.updatePurchaseStatusService(transactionId: transactionId, authCode: authCode, responseCode: responseCode, amount: amount, paymentStatus: paymentStatus, paymentResponse: paymentResponse)
                                }else{
                                    //Make Toast
                                    self.view.makeToast("\(objProfileRes.paymentResponse!)")
                                }
    ...
    func updatePurchaseStatusService(transactionId: String, authCode:String, responseCode:String, amount:String, paymentStatus:String, paymentResponse:String) {
        PKHUD.sharedHUD.contentView = PKHUDProgressView()
        PKHUD.sharedHUD.show()
   ...
   /* func updatePurchaseStatusService_Second(transactionId: String, authCode:String, responseCode:String, amount:String, paymentStatus:String, paymentResponse:String) {
        PKHUD.sharedHUD.contentView = PKHUDProgressView()
        PKHUD.sharedHUD.show()
